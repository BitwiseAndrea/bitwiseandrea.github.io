(function(window) {
  ...
  p.onTouchMove = function(e) {
      ...
      var a = Math.abs(Math.atan2(dy, dx) / Math.PI * 180);
  ...
  p.onClick = function(e) {
      log("onClick");
      SoundController.play("sounds/birds", {
          loop: true
      });
  }
  ...
  p.setSize = function(w, h, dpr) {
      ...
      this.bg.scaleX = this.canvas.width / ref.width;
  ...
  p.updateRain = function(rx, ry) {
      ...
      for (var i = 0; i < this.drops.length; i++) {
          ...
          drop.x = drop.dx + drop.rx * this.canvas.width;
  ...
  p.addRainDrop = function(r) {
      ...
      drop.rx = (1 + 2 * margin) * Math.random() - (.5 + margin);
      drop.dx = 0;
